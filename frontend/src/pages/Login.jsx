...
import { UtensilsCrossed, Users, Wallet, GraduationCap, User, Lock, Eye, EyeOff, Phone } from "lucide-react";
import { useAuth } from "../context/AuthContext";
import parentApi from "../api/parentAxios";

const FEATURES = [
  { icon: UtensilsCrossed, label: "Cuntada Maalinlaha" },
  { icon: GraduationCap, label: "Diiwaanka Ardayda" },
  { icon: Wallet, label: "Lacagaha Billaha" },
  { icon: Users, label: "Xisaabta Waalidka" },
];
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {parentMode === "register" && (
          <div className="bg-navy/5 text-ink/70 text-sm rounded-md px-3 py-2">
            Waxaad dhigaysaa password aad isticmaali doonto marar dambe. Lambarkaagu waa inuu horey ugu jiraa maqaayda (maamulku waa uu diiwaan geliyay).
          </div>
        )}
      ...
      <div className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-sm">
          ...
          <p className="text-center text-xs text-ink/40 mt-8">
            © {new Date().getFullYear()} Taysiir International School
          </p>
        </div>
      </div>
    </div>
